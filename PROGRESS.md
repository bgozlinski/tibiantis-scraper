...
- 🔵 #81 [M4-D21] Celery `scrape_deaths` task + PeriodicTask seed migration + `DEATH_LEVEL_THRESHOLD` env — OPEN
- 🔵 #82 [M4-D22] `recentDeaths` GraphQL query + e2e + zamknięcie M4 — OPEN
...
## 🚧 Milestone M4 — Deaths monitor (backend) — IN PROGRESS (since 2026-05-05)
Design spec: [`docs/superpowers/specs/2026-04-30-m4-deaths-monitor-design.md`](docs/superpowers/specs/2026-04-30-m4-deaths-monitor-design.md). Implementation plan: [`docs/superpowers/plans/2026-04-30-m4-implementation-plan.md`](docs/superpowers/plans/2026-04-30-m4-implementation-plan.md). 3/5 zadań ukończonych — D18 model, D19 spider, D20 service+cmd. Zostają D21 (Celery task) + D22 (GraphQL).
...
- **`celery-types` package** (carry-over z M3 tech debt) — wciąż otwarte. M4-D21 doda kolejny Celery task (`scrape_deaths`), więc wzorzec mypy `[no-untyped-decorator]` na `@shared_task` może uderzyć ponownie. Rozważyć przed D21.
